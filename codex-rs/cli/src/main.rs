...
enum Subcommand {
    ...
    /// [experimental] Launch the macOS Tokonix overlay app.
    #[clap(name = "overlay", alias = "voice-overlay")]
...
struct OverlayCommand {
    /// Path to a Tokonix app bundle (.app). Overrides TOKONIX_OVERLAY_APP if set.
    #[arg(long = "app", value_name = "PATH")]
    app_path: Option<PathBuf>,

    /// Repository root to search for `macos-tokonix/dist/Tokonix.app`.
...
fn run_overlay_command(cmd: OverlayCommand) -> anyhow::Result<()> {
    #[cfg(target_os = "macos")]
    {
        run_overlay_command_macos(cmd)
    }

    #[cfg(not(target_os = "macos"))]
    {
        let _ = cmd;
        anyhow::bail!("The Tokonix overlay app is only supported on macOS.");
    }
}

#[cfg(target_os = "macos")]
fn run_overlay_command_macos(cmd: OverlayCommand) -> anyhow::Result<()> {
    let env_override = std::env::var_os("TOKONIX_OVERLAY_APP")
        .filter(|value| !value.is_empty())
        .map(PathBuf::from);
    let app_override = cmd.app_path.clone().or(env_override);
    let repo_root = resolve_overlay_repo_root(cmd.repo_root.as_deref());

    if let Some(app_path) = app_override {
        if cmd.build {
            eprintln!("Ignoring --build because an explicit app path was provided.");
        }
        return open_overlay_app(&app_path);
    }

    if cmd.build {
        let Some(root) = repo_root.as_deref() else {
            anyhow::bail!("--build requires a repository root containing macos-tokonix.");
        };
        build_overlay_app(root)?;
    }

    for candidate in overlay_app_candidates(repo_root.as_deref()) {
        if candidate.exists() {
            return open_overlay_app(&candidate);
        }
    }

    anyhow::bail!(
        "Unable to locate Tokonix.app. Set TOKONIX_OVERLAY_APP, pass --app PATH, or run from the repo root."
    )
}

#[cfg(target_os = "macos")]
fn resolve_overlay_repo_root(repo_root: Option<&Path>) -> Option<PathBuf> {
    if let Some(root) = repo_root
        && root.join("macos-tokonix").is_dir()
    {
        return Some(root.to_path_buf());
    }

    if let Ok(cwd) = std::env::current_dir()
        && let Some(found) = find_overlay_repo_root_from(&cwd)
    {
        return Some(found);
    }

    if let Ok(exe) = std::env::current_exe() {
        return find_overlay_repo_root_from(&exe);
    }
...
#[cfg(target_os = "macos")]
fn find_overlay_repo_root_from(path: &Path) -> Option<PathBuf> {
    for ancestor in path.ancestors() {
        if ancestor.join("macos-tokonix").is_dir() {
...
#[cfg(target_os = "macos")]
fn overlay_app_candidates(repo_root: Option<&Path>) -> Vec<PathBuf> {
    let mut candidates = Vec::new();
    if let Some(root) = repo_root {
        candidates.push(root.join("macos-tokonix").join("dist").join("Tokonix.app"));
    }
    candidates.push(PathBuf::from("/Applications/Tokonix.app"));
    if let Some(home) = std::env::var_os("HOME") {
        candidates.push(PathBuf::from(home).join("Applications/Tokonix.app"));
    }
    candidates
}

#[cfg(target_os = "macos")]
fn build_overlay_app(repo_root: &Path) -> anyhow::Result<()> {
    let overlay_dir = repo_root.join("macos-tokonix");
    let script_path = overlay_dir.join("build_app.sh");
    if !overlay_dir.is_dir() {
        anyhow::bail!(
            "Expected macos-tokonix at {}, but it was not found.",
            overlay_dir.display()
        );
    }
    if !script_path.is_file() {
        anyhow::bail!(
            "Expected build script at {}, but it was not found.",
            script_path.display()
        );
    }
    let status = std::process::Command::new(&script_path)
        ...
        .status()?;
    if !status.success() {
        anyhow::bail!("Overlay build failed with status {status}");
    }
    Ok(())
}

#[cfg(target_os = "macos")]
fn open_overlay_app(app_path: &Path) -> anyhow::Result<()> {
    let status = std::process::Command::new("open").arg(app_path).status()?;
    if !status.success() {
        anyhow::bail!("Failed to open {}", app_path.display());
    }
    Ok(())
}
...
async fn cli_main(codex_linux_sandbox_exe: Option<PathBuf>) -> anyhow::Result<()> {
    ...
    match subcommand {
        ...
        Some(Subcommand::Overlay(overlay_cli)) => {
            run_overlay_command(overlay_cli)?;
        }
