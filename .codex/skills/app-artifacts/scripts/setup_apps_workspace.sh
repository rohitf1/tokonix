...
DEST_ROOT=${1:-"$CODEX_HOME_DIR/skill-workspaces/artifacts/apps"}
APP_SLUG=${2:-""}
...
if [[ -z "$APP_SLUG" ]]; then
  echo "Usage: setup_apps_workspace.sh <root> <app_slug>"
  exit 1
fi

SCRIPT_DIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
TEMPLATE_DIR="$SCRIPT_DIR/../assets/apps-template"
DEST_DIR="$DEST_ROOT/$APP_SLUG"

if [[ -d "$DEST_DIR" ]]; then
  echo "Apps workspace already exists: $DEST_DIR"
  exit 0
fi
...
echo "Apps workspace ready at $DEST_DIR"
